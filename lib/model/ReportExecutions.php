...
/**
 * ReportExecutions Class Doc Comment
 *
 * @category Class
 ...
 */
class ReportExecutions implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ReportExecutions';
    ...
    protected static $openAPITypes = [
        'items' => '\Gr4vy\model\ReportExecution[]',
        'limit' => 'int',
        'next_cursor' => 'string',
     ...
     * Gets items
     *
     * @return \Gr4vy\model\ReportExecution[]|null
     */
     ...
     * Sets items
     *
     * @param \Gr4vy\model\ReportExecution[]|null $items A list of report executions.
     *
     * @return self
     */
    ...
    public function setLimit($limit)
    {
        ...
        if (!is_null($limit) && ($limit > 100)) {
            throw new \InvalidArgumentException('invalid value for $limit when calling ReportExecutions., must be smaller than or equal to 100.');
        }
        if (!is_null($limit) && ($limit < 1)) {
            throw new \InvalidArgumentException('invalid value for $limit when calling ReportExecutions., must be bigger than or equal to 1.');
        }
    ...
    public function setNextCursor($next_cursor)
    {
        if (!is_null($next_cursor) && (mb_strlen($next_cursor) > 1000)) {
            throw new \InvalidArgumentException('invalid length for $next_cursor when calling ReportExecutions., must be smaller than or equal to 1000.');
        }
        if (!is_null($next_cursor) && (mb_strlen($next_cursor) < 1)) {
            throw new \InvalidArgumentException('invalid length for $next_cursor when calling ReportExecutions., must be bigger than or equal to 1.');
        }
    ...
    public function setPreviousCursor($previous_cursor)
    {
        if (!is_null($previous_cursor) && (mb_strlen($previous_cursor) > 1000)) {
            throw new \InvalidArgumentException('invalid length for $previous_cursor when calling ReportExecutions., must be smaller than or equal to 1000.');
        }
        if (!is_null($previous_cursor) && (mb_strlen($previous_cursor) < 1)) {
            throw new \InvalidArgumentException('invalid length for $previous_cursor when calling ReportExecutions., must be bigger than or equal to 1.');
        }
