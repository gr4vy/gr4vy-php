...
class PaymentServiceDefinitions
{
    ...
    public function session(array $requestBody, string $paymentServiceDefinitionId, ?Options $options = null): CreatePaymentServiceDefinitionSessionResponse
    {
        ...
        $hookContext = new HookContext($baseUrl, 'create_payment_service_definition_session', [], $this->sdkConfiguration->securitySource);
    ...
    public function get(string $paymentServiceDefinitionId, ?Options $options = null): GetPaymentServiceDefinitionResponse
    {
        ...
        $hookContext = new HookContext($baseUrl, 'get_payment_service_definition', [], $this->sdkConfiguration->securitySource);
        $httpRequest = $this->sdkConfiguration->hooks->beforeRequest(new Hooks\BeforeRequestContext($hookContext), $httpRequest);
        $httpOptions = Utils\Utils::convertHeadersToOptions($httpRequest, $httpOptions);
        $httpRequest = Utils\Utils::removeHeaders($httpRequest);
        try {
    ...
    private function listIndividual(?string $cursor = null, ?int $limit = null, ?Options $options = null): ListPaymentServiceDefinitionsResponse
    {
        ...
        $httpOptions['headers']['user-agent'] = $this->sdkConfiguration->userAgent;
        $httpRequest = new \GuzzleHttp\Psr7\Request('GET', $url);
        $hookContext = new HookContext($baseUrl, 'list_payment_service_definitions', [], $this->sdkConfiguration->securitySource);
        $httpRequest = $this->sdkConfiguration->hooks->beforeRequest(new Hooks\BeforeRequestContext($hookContext), $httpRequest);
        ...
        if (Utils\Utils::matchStatusCodes($statusCode, ['200'])) {
            if (Utils\Utils::matchContentType($contentType, 'application/json')) {
                ...
                $serializer = Utils\JSON::createSerializer();
                $responseData = (string) $httpResponse->getBody();
                $obj = $serializer->deserialize($responseData, '\Gr4vy\CollectionPaymentServiceDefinition', 'json', DeserializationContext::create()->setRequireAllRequiredProperties(true));
